...
import { Target, AlertTriangle, CheckCircle2, TrendingUp, Wallet } from "lucide-react";
...
export default function PlanificacionGastosView() {
    ...
    const [presupuesto, setPresupuesto] = useState<number>(2000);
    ...
    if (isLoading) return <div className="h-screen w-full flex items-center justify-center bg-[#141414]"><div className="animate-spin rounded-full h-12 w-12 border-t-2 border-emerald-500"></div></div>;
        ...
        <div className="p-4 md:p-8 bg-[#141414] min-h-screen text-white">
            <div className="max-w-6xl mx-auto flex flex-col gap-8">
                ...
                <div className="flex flex-col gap-1">
                    <h1 className="text-4xl font-black tracking-tighter italic">PLANIFICACIÓN</h1>
                    <p className="text-zinc-500 text-sm font-medium">Comparativa de base de datos vs. presupuesto objetivo.</p>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    <div className="lg:col-span-5 bg-[#1e1e1e] border border-[#2a2a2a] rounded-[2.5rem] p-8 flex flex-col items-center justify-center relative min-h-[400px]">
                        ...
                        <div className="absolute flex flex-col items-center justify-center pointer-events-none">
                            <span className="text-zinc-500 text-[10px] font-black uppercase tracking-widest">Consumido</span>
                    ...
                    <div className="lg:col-span-7 flex flex-col gap-6">
                        <div className="bg-[#1e1e1e] border border-[#2a2a2a] p-8 rounded-[2.5rem]">
                            <div className="flex items-center gap-4 mb-6">
                                ...
                                <div>
                                    <h2 className="text-xl font-bold">Presupuesto Objetivo</h2>
                                    <p className="text-zinc-500 text-xs">Define cuánto planeas gastar este mes.</p>
                                </div>
                            </div>
                            ...
                            <div className="relative group">
                                ...
                                <input
                                    ...
                                    className="w-full bg-[#141414] border-2 border-[#2a2a2a] group-hover:border-emerald-500/50 p-6 pl-14 rounded-2xl text-3xl font-black text-white outline-none transition-all focus:border-emerald-500 shadow-inner"
                                />
                            </div>
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                            <div className="bg-[#1e1e1e] border border-[#2a2a2a] p-6 rounded-3xl">
                                <p className="text-zinc-500 text-[10px] font-black uppercase mb-2">Gasto Real (DB)</p>
                                <p className="text-2xl font-black text-white">S/. {gastoTotal.toLocaleString()}</p>
                            </div>
                            <div className="bg-[#1e1e1e] border border-[#2a2a2a] p-6 rounded-3xl">
                                <p className="text-zinc-500 text-[10px] font-black uppercase mb-2">Margen Restante</p>
                                <p className={`text-2xl font-black ${margen < 0 ? 'text-red-500' : 'text-emerald-500'}`}>
                                    S/. {margen.toLocaleString()}
                                </p>
                            </div>
                        </div>
                        <div className={`p-6 rounded-3xl border ${status.bg} ${status.border} flex items-start gap-4`}>
                            <div className={status.color}>{status.icon}</div>
                            <div className="flex flex-col gap-1">
                                <h4 className={`font-black uppercase text-xs tracking-wider ${status.color}`}>{status.label}</h4>
                                <p className="text-sm text-zinc-300 leading-relaxed">
                                    {porcentaje >= 100
                                        ? `Te has excedido por S/. ${Math.abs(margen)}. Como estudiante de ingeniería, sabes que esto afecta tu flujo de caja. ¡Revisa tus registros!`
                                        : `Vas por buen camino. Te queda el ${(100 - porcentaje).toFixed(1)}% de tu presupuesto para terminar el mes con éxito.`}
                                </p>
                            </div>
                        </div>
                    </div>
