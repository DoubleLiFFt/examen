...
export default function DetailsTable() {
    ...
    const [editarGasto, setEditarGasto] = useState<any>(null);
    const [modalOpen, setModalOpen] = useState(false)

    const eliminarGasto = (idEliminar : number) => {
        const newList = listaGastos.filter(gasto => gasto.id !== idEliminar)
        setListaGastos(newList)
    }
    const prepararEdit = (gasto: any)=> {
        setEditarGasto(gasto)
        setModalOpen(true)
    }
    const guardarCambios = (newData: any) => {
        const listaAct = listaGastos.map((gasto) =>
            gasto.id === editarGasto.id ? { ...gasto, ...newData } : gasto
        );
        setListaGastos(listaAct);
        setModalOpen(false);
    };
    return (
                        ...
                        >
                            ...
                            <td className="px-2 md:px-4 py-4">
                                <div className="flex justify-center items-center gap-2 md:gap-3">
                                    <button onClick={() => prepararEdit(Details)} className="text-blue-500 hover:text-blue-400 transition-all hover:scale-110 p-1 hover:cursor-pointer">
                                        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 md:h-5 md:w-5" viewBox="0 0 20 20" fill="currentColor">
                                            <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" />
                                        </svg>
                                    </button>
                                    <button onClick={ () => eliminarGasto(Details.id) } className="text-red-500 hover:text-red-400 transition-all hover:scale-110 p-1 hover:cursor-pointer">
